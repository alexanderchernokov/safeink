...
function LoginUser($loginusername, $loginpassword)
{
  ...
  $loginpassword = ipb_clean($loginpassword);
	...
	if(strlen($loginusername))
	{
		...
		if($getuser = $DB->query_first('SELECT * FROM ' . $usersystem['tblprefix'] . "members WHERE name = '%s'", $loginusername))
		{
			if($converge = $DB->query_first('SELECT converge_pass_hash, converge_pass_salt
        FROM ' . $usersystem['tblprefix'] . "members_converge
        WHERE converge_id = %d AND converge_email = '%s'",
        $getuser['id'], $getuser['email']))
			{
...
function GetUser($userid)
{
  ...
  $user = NULL;
	$getuser = $DB->query_first('SELECT * FROM ' . $usersystem['tblprefix'] . 'members WHERE id = %d', $userid);
	if(!empty($getuser['id']))
	{
    $user = array(
      'userid'         => $getuser['id'],
