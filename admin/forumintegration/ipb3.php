...
function GetForumSystem($forumfolderpath, $sddbname)
{
  ...
  if(is_file(ROOT_PATH . $forumfolderpath . 'conf_global.php'))
  {
    ...
    if(include(ROOT_PATH . $forumfolderpath . 'conf_global.php'))
    ...
    if(isset($INFO) && is_array($INFO) && $DB->select_db($INFO['sql_database'],true) !== false)
    {
      echo 'Connected to forum database.<br />';
      $conf_table = 'conf_settings';
      if($DB->query_first("SHOW TABLES LIKE '".$INFO['sql_tbl_prefix']."core_sys_conf_settings'"))
      {
        $conf_table = 'core_sys_conf_settings';
      }
      // default values are empty, so just get the entered settings
      $cookieprefix  = $DB->query_first('SELECT conf_value FROM ' . $INFO['sql_tbl_prefix'] . $conf_table .
                                        " WHERE conf_key = 'cookie_id'");

      // however this has a normal default of 3600, so use that if the entered info is empty
      $cookietimeout = $DB->query_first('SELECT conf_value FROM ' . $INFO['sql_tbl_prefix'] . $conf_table .
                                        " WHERE conf_key = 'session_expiration'");
      $cookietimeout['conf_value'] = strlen($cookietimeout['conf_value']) ? $cookietimeout['conf_value'] : 3600;

      $cookiedomain = $DB->query_first('SELECT conf_value FROM ' . $INFO['sql_tbl_prefix'] . $conf_table .
                                       " WHERE conf_key = 'cookie_domain'");
      $cookiedomain['conf_value'] = trim($cookiedomain['conf_value']);

      $cookiepath = $DB->query_first('SELECT conf_value FROM ' . $INFO['sql_tbl_prefix'] . $conf_table .
                                     " WHERE conf_key = 'cookie_path'");
      ...
      */
      ...
      $forumsystem = array('name'          => 'Invision Power Board 3',
