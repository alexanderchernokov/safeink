...
{
  if(empty($package['ip'])) return false;
	if (@is_ipv6($package['ip'])) return false;	# TODO
	if (match_cidr($package['ip'], array("66.102.0.0/20", "66.249.64.0/19", "66.249.71.4", "66.249.71.216", "64.233.160.0/19", "72.14.192.0/18", "203.208.32.0/19", "74.125.0.0/16", "216.239.32.0/19", "209.85.128.0/17")) === FALSE) {
		return "f1182195";
	}
...
function bb2_msnbot($package)
{
  if(empty($package['ip'])) return false;
	if (@is_ipv6($package['ip'])) return false;	# TODO
	if (match_cidr($package['ip'],
    array("131.253.0.0/16", //2012-09-10
          "199.30.16.0/20", //2013-02-21
          "207.46.0.0/16",
          "65.52.0.0/14",
          "207.68.128.0/18",
          "207.68.192.0/20",
