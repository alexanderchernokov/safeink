...
{
	if (empty($settings['logging'])) return;
...
function bb2_display_denial($settings, $package, $key, $previous_key = false)
{
  ...
  $response['explanation'] = empty($response['explanation'])?'':$response['explanation'];
  $response['response'] = empty($response['response'])?'':$response['response'];
