...
function bb2_cookies($settings, $package)
{
	...
	if (isset($package['headers_mixed']['Cookie']) &&
      (@strpos($package['headers_mixed']['Cookie'], '$Version=0') !== FALSE) &&
      (!isset($package['headers_mixed']) ||
       !array_key_exists('Cookie2', $package['headers_mixed'])) &&
      (!isset($package['headers_mixed']['User-Agent']) ||
       strpos($package['headers_mixed']['User-Agent'], "Kindle/") === FALSE) )
  {
		return '6c502ff1';
	}
	return false;
...
{
	@$ua = isset($package['headers_mixed']['User-Agent'])?$package['headers_mixed']['User-Agent']:'';
  ...
  #SD: problematic, catches valid users
  /*
  ...
  */
  if(!empty($package['headers_mixed']['Referer']) &&
	   array_key_exists('Referer', $package['headers_mixed']))
  {
