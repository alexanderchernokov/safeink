...
*/
  ...
  var $prefer_curl = true; //SD 2013-10-08
  var $exit_on_ajax = false; //SD 2013-10-12
...
*/
  ...
  Function SendAPIRequest($url, $method, $parameters, $oauth, $options, &$response)
  {
    ...
    $http->log_debug = true;
    $http->prefer_curl = $this->prefer_curl; //SD
    ...
    switch($type)
    {
      ...
      case 'multipart/form-data':
        ...
        if(isset($options['RequestContentType']))
        $arguments['Headers']['Content_Type'] = $options['RequestContentType']; //fix from forum
        break;
      case 'application/json':
        $arguments['Headers']['Content-Type'] = $options['RequestContentType'];
        if(!IsSet($options['RequestBody']) && !empty($parameters)) //fix from forum
        {
