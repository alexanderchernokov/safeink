...
$vblicensenumber = $usersystem['extra']; // the extra column holds vb3's license number
...
// ####################### GET VBULLETIN 3 SESSION LIMIT #######################
// updating to the vbulletin3's session table will not take place if the limit has been reached

$getsessionlimit = $DB->query_first("SELECT value FROM " . $tableprefix . "setting WHERE varname = 'sessionlimit'");
$sessionlimit    = isset($getsessionlimit['value']) ? $getsessionlimit['value'] : 0;  // this value should be store in $usersystem['extra'], wasted query imo
...
if(empty($login_errors_arr) && isset($_POST['login']) && ($_POST['login'] == 'login'))
{
  ...
  if(isset($loginusername{0}))
  {
    ...
    {
      if($getuser['password'] != md5(md5($loginpassword) . vblicensenumber) )
      {
...
if($usersettings['userid'] != 0)
{
  ...
  $vb3userinfo['logouthash'] = md5($vb3userinfo['userid'] . $vblicensenumber);
...
if(!defined('SD_VB_INCLUDED2'))
{
  ...
  function ForumAvatar($userid, $username)
  {
    ...
    if($version[0] == 3)
    {
      ...
      {
        $query = 'SELECT user.avatarid, user.avatarrevision, avatarpath, NOT ISNULL(filedata) AS hascustom,
    ...
    {
      $avatar = 'This doesn\'t appear to be vBulletin 3. The version number is ' . $ver[0];
    }
    ...
    if ($avatarinfo = $DB->query_first($query))
    {
      ...
      {
        $avatar = '<img alt="avatar" src="' . $sdurl . $forumpath . $avatarinfo['avatarpath'] . '" />';
      }
      else if (!empty($avatarinfo['hascustom']) || !empty($avatarinfo['filename']))
      {
        ...
        if(isset($usefileavatar[0]) && $usefileavatar[0])
        {
          ...
          $avatar = '<img alt="avatar" src="' . $avurl . '/avatar' . $avatarinfo['userid'] . '_' . $avatarinfo['avatarrevision'] . '.gif" />';
        }
        else
        {
          $avatar = '<img alt="avatar" src="' . $sdurl . $forumpath . 'image.php?u=' . $avatarinfo['userid'] . '&amp;dateline=' . $avatarinfo['dateline'] . '" />';
        }
      }
    }
    else
