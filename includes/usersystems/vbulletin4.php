...
$cookieprefix = strlen($cookieprefix) ? $cookieprefix.'_' : 'bb';
...
// ################################## DEFINES ##################################
define('SESSION_IDHASH',  mysql_escape_string(md5($_SERVER['HTTP_USER_AGENT'] . ALT_IP)));
...
if(!defined('SD_VB_INCLUDED2'))
{
  ...
  function ForumAvatar($userid, $username)
  {
    ...
    {
      $extrasql = 'WHERE user.username = "' . $DB->escape_string($username) . '"';
    }
    ...
    if ($avatarinfo = $DB->query_first($query))
    {
      ...
      {
        $avatar = '<img alt=" " src="' . $sdurl . $forumpath . $avatarinfo['avatarpath'] . '" />';
      }
      else if (!empty($avatarinfo['hascustom']) || !empty($avatarinfo['filename']))
      {
        ...
        if(isset($usefileavatar[0]) && $usefileavatar[0])
        {
          ...
          $avatar = '<img alt=" " src="' . $avurl . '/avatar' . $avatarinfo['userid'] . '_' . $avatarinfo['avatarrevision'] . '.gif" />';
        }
        else
        {
          $avatar = '<img alt=" " src="' . $sdurl . $forumpath . 'image.php?u=' . $avatarinfo['userid'] . '&amp;dateline=' . $avatarinfo['dateline'] . '" />';
        }
      }
    }
...
if(!defined('SD_VB_INCLUDED'))
{
  ...
  function CreateSession($userid = 0)
  {
    ...
    $DB->query('INSERT INTO ' . $tableprefix . "session (sessionhash, userid, host, useragent, idhash, lastactivity)
                ...
                $DB->escape_string($session['useragent']), $session['idhash'], $session['lastactivity']);
...
{
  $vb4userinfo = $DB->query_first('SELECT * FROM ' . $tableprefix . 'user WHERE userid = %d', $usersettings['userid']);

  if(isset($_COOKIE['bbstyleid']))
  {
    $vb4userinfo['styleid'] = $_COOKIE['bbstyleid'];
  }

  //vb4: see function fetch_userinfo in "vb/includes/functions.php"
  //SD343: fill $usersettings with values, not $user
  $usersettings['securitytoken_raw'] = sha1($vb4userinfo['userid'] . sha1($vb4userinfo['salt']) . sha1(COOKIE_SALT));
  $usersettings['securitytoken'] = TIMENOW . '-' . sha1(TIMENOW . $usersettings['securitytoken_raw']);
  $usersettings['salt'] = $vb4userinfo['salt'];

  $vb4userinfo['logouthash'] = $user['securitytoken'];// changed in 3.8
}
