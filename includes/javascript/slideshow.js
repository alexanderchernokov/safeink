jQuery(document).ready(function(){
  // tobias: "li:first" and "li:last" must have "#slideShow ul " as selector start!
	var slideShow = jQuery('#slideShow'),
		ul = jQuery('#slideShow ul'),
		li = jQuery('#slideShow ul').find('li'),
		cnt = li.length;
		...
		slideShow.bind('rotateContainer',function(e,direction,degrees){
			...
			slideShow.animate({
				...
				if(direction == 'next'){
					...
					$('#slideShow ul li:first').fadeOut('slow',function(){
					...
					var liLast = $('#slideShow ul li:last').hide().remove().prependTo(ul);
					updateZindex();
					liLast.fadeIn('slow');
				}
