...
$.fn.ceebox.defaults = {
	...
	animSpeed: "fast", /* animation resize transition speed (can be set to "slow","normal","fast", or in milliseconds like 1000) */
	easing: "swing", /* supports use of the easing plugin for resize anim (http://gsgd.co.uk/sandbox/jquery/easing/) */
	fadeOut: 200, /* speed that ceebox fades out when closed or advancing through galleries */
	fadeIn: 200, /* speed that ceebox fades in when opened or advancing through galleries */
	overlayColor:"#000",
	overlayOpacity:0.9,
  ...
  itemCaption: "Item", /* Tobias 2010-11-08: caption of the "Item" in title (Item x of y) */
  ofCaption: "of", /* Tobias 2010-11-08: caption of the "of" in title (Item x of y) */
  galleryClass: "", /* Tobias 2010-11-06: only use elements with this class in gallery selection */
};
/* ratio shortcuts */
...
This function is called ahead of time so the loader is there, but it does not
have to be called as the ceebox.popup script also calls this */
...
$.fn.ceebox.popup = function(content,opts) {
	...
	if ($(content).is("a,area,input") && (opts.type == "html" || opts.type == "image" || opts.type == "video")) {
		...
		if (opts.gallery) {
      if(opts.galleryClass=="") {
        /* default CeeBox behavior: select all links with href of parentId-element */
        family = $(opts.selector).eq(opts.gallery.parentId).find("a[href],area[href],input[href]");
      }
      else
      {
        /* Tobias 2010-11-06: new behavior: select all links with given class name of parentId-element */
        family = $(opts.selector).eq(opts.gallery.parentId).find("a[href],area[href],input[href]").has(opts.galleryClass);
      }
    }
...
/* --------------------------- ceebox onload function ------------------------
   made a public function mainly for cee_iframe to call. $.fn.ceebox.popup calls
   this automatically once all children objects of the popup are loaded */
$.fn.ceebox.onload = function(opts){
		$("#cee_load").hide(200).fadeOut(500,function(){$(this).remove();}); /*  remove loading anim */
...
function init(elem,opts,selector) {
    ...
    if(opts.galleryClass!="") tgt = $(tgt).has(opts.galleryClass); /* Tobias: added ".has" */
...
var ceeboxLinkSort = function(parent,parentId,opts,selector) {
  ...
  // Tobias 2010-11-06: new behavior: select all links with given class name (if specified) */
  if(!opts.galleryClass || opts.galleryClass=="") {
    ($(parent).is("[href]")) ? family = $(parent) : family = $(parent).find("[href]");
  } else {
    ($(parent).is("[href]")) ? family = $(parent) : family = $(parent).find("[href]").has(opts.galleryClass);
  }
...
function addGallery(g,family,opts){ /* adds gallery next/prev functionality */
	...
	function navLink(btn,id) {
		...
		var style = function(y) {return $.extend({zIndex:105,top:0,width:nav[opts.type].w + px, height:nav[opts.type].h + px,position:"absolute",top:nav[opts.type].top,backgroundPosition:s[1] + " " + y + px},s[0]);};
	...
	$("#cee_title").append("<div id='cee_count'>"+opts.itemCaption+" "+ (g.gNum+1) + opts.ofCaption + " "+ g.gLen + "</div>");
}
